...
# The sim_NN.yml files generated by expand_design.sh sit alongside the
# exp_vX.yml, so its directory is the config dir bundle_exp.R needs to build
# the per-sim design table. Without it the bundle has no record of which
# factors the experiment swept.
CONFIG_DIR="$(cd "$(dirname "$EXP_YML")" && pwd)"

Rscript R/bundle_exp.R "$EXP_RUN_DIR" "$CONFIG_DIR"
