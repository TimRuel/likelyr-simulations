...
# Returns a list so that derived quantities (J, n_obs) can be
# forwarded to the parameter spec.
# ----------------------------------------------------------------------
...
generate_eta_0_manual <- function(param_cfg) {
  ...
  # Sum before normalisation: if values > 1 treat as counts and record n_obs,
  # otherwise record n_obs as NA (probabilities carry no implied sample size).
  is_counts <- any(val > 1)
